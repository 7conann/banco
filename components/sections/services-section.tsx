...
export function ServicesSection() {
  ...
  const services = [
    {
      ...
      title: "Conta Nominal",
      description:
        "Gestão simplificada para múltiplos beneficiários com maior privacidade e facilidade administrativa.",
      badge: "Gestão",
    ...
    },
    {
      ...
      title: "Boleto PIX",
      description:
        "Emissão de boletos com pagamento via PIX instantâneo ou método tradicional com máxima flexibilidade.",
      badge: "Rapidez",
      badgeColor: "bg-primary text-black",
    },
    {
  ...
  const desktopLogos = [...partnerLogos, ...partnerLogos, ...partnerLogos]
  const mobileLogos = [
    ...partnerLogos,
    ...partnerLogos,
    ...partnerLogos,
    ...
    <section className="bg-gray-50">
      <div className="relative bg-white overflow-hidden h-28">
        <div className="flex items-center h-full">
          <div className="hidden md:flex items-center gap-8 md:gap-10 animate-scroll">
            {desktopLogos.map((p, i) => (
              <Image
                key={i}
                ...
                height={100}
                draggable={false}
                className="h-8 md:h-10 w-auto object-contain opacity-90 hover:opacity-100 transition flex-shrink-0"
                priority={i < 6}
              />
            ))}
          </div>

          <div className="flex md:hidden items-center gap-4 animate-scroll">
            {mobileLogos.map((p, i) => (
              <Image
                key={i}
                src={p.src || "/placeholder.svg"}
                ...
                height={100}
                draggable={false}
                className="h-8 w-auto object-contain opacity-90 hover:opacity-100 transition flex-shrink-0"
                priority={i < 6}
              />
            ))}
          </div>
          ...
          {services.map((service, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 bg-primary rounded-lg flex items-center justify-center">{service.icon}</div>
